...
```{r, echo=FALSE}
# Distribution of LOS (length of stay)
plot1 <- densityplot(~drug.data$LOS|drug.data$TREATCOMPLETE, 
                  main="Density Plot of Treatment Completion by Length of Stay",
                  xlab="Length of Stay",cex=0.75,
                  layout=c(1,2))
plot1
```
...
rownames(as.matrix(coef.min)) # AGE, GENDER, RACE, ETHNIC, MARSTAT, EDUC, EMPLOY, VET, LIVARAG, PRIMINC, DIVISION, SERVSETD, METHUSE, DAYWAIT, LOS, PSOURCE, SUB1, ROUTE1, FREQ1, FRSTUSE1, NUMSUBS, ALCDRUG, IDU, PSYPROB, PC1, PC2, PC3, PC5, PC6 (29 vars)

# Step 3. Look at lambda.1se for minimizing deviance:
coef.1se <- coef(fit.cv, s="lambda.1se")  
coef.1se <- coef.1se[which(coef.1se !=0),] 
rownames(as.matrix(coef.1se)) # AGE, GENDER, RACE, ETHNIC, MARSTAT, EDUC, EMPLOY, VET, LIVARAG, PRIMINC, DIVISION, SERVSETD, METHUSE, DAYWAIT, LOS, PSOURCE, SUB1, ROUTE1, FREQ1, FRSTUSE1, NUMSUBS, ALCDRUG, IDU, PSYPROB, PC1, PC2, PC3, PC6 (28 vars)
...
P.Regression.1.fittedprobabilities <- predict(P.Regression.1, drugdata.test, type="response")  #Get probabilities
P.Regression.1.classify <- ifelse(P.Regression.1.fittedprobabilities > 0.5, "1", "0") #Classify

P.Regression.2.fittedprobabilities <- predict(P.Regression.2, drugdata.test, type="response")  #Get probabilities
...
fit.rf.mtry10.pred <- predict(fit.rf.mtry10, drugdata.test, type="prob")  
fit.rf.mtry10.pred.y <- predict(fit.rf.mtry10, drugdata.test, type="response")
fit.rf.mtry10.train.err <- mean(drugdata.test$TREATCOMPLETE != fit.rf.mtry10.pred.y) # 0.26405

# Tuning ntree parameter:
fit.rf.ntree<- randomForest(TREATCOMPLETE ~ ., data = drugdata.train, mtry = 10, importance = T, ntree=500)
plot(fit.rf.ntree)

# Growing the forest with optimized parameters
fit.rf <- randomForest(TREATCOMPLETE ~ ., data = drugdata.train, mtry = 10, importance = T, ntree=400)

#save(fit.rf, file="./randomForest_model.Rdata") # Save out final model
save(fit.rf, file="./randomForest_model_02.Rdata") # Save out final model

# Can better look at model perfomance in testing data
drugdata.test$TREATCOMPLETE <- as.factor(drugdata.test$TREATCOMPLETE)
...
fit.rf.train.err <- mean(drugdata.test$TREATCOMPLETE != fit.rf.test.pred.y) # 0.2634
...
Specifically, we note that factors like `SERVSETD` (service setting at discharge), `PSOURCE` (principal source of referral), `FREQ1` (frequency of use), `PSYCPROB` (psychiatric problems/symptoms/diagnoses), and `PRIMINC` (source of income/support) are most predictive of successful treatment completion. On the other hand, factors such as `AGE` (age at time of admission), `SUB1` (substance problem code), `FRSTUSE1` (age at first substance use), and `DAYWAIT` (days spent waiting for admission) were most predictive of cases where treatment was not completed. 
...
load("./randomForest_model_02.Rdata")
...
Predictor importance is only part of the story. In addition to knowing the importance of each inout, it may be useful to have a description of how each predictor is related to the response. One way to accomplish this is through partial dependence plots. 

For tree-based approaches, these dependence plots show the average relationship between a given input and the response within the fixed, joint distribution of the other inputs. In other words, these plots show how what what the proportion for a particular classification outcome would be for different values of the variable of interest. Consider the variable `DIVISION`, which is the most important predictor. Since there are 7 unique values for number of inpatient visits, the algorithm fits 7 models where in each one all the values for DIVISION are the same. This allows us to gain some insight into how the particular variable affects the prediction. Specifically, we see that 
...
```{r Boosting, eval=FALSE, include=FALSE}
# Step 1. Format Data
...
```{r Boosting GBM, eval=FALSE, include=FALSE}
# Step 1. Format Data
...
## MCE
MCE.NN <- 1-results$acc # MCE = 0.332

NN_predict_classes <- predict_classes(object = model, x = as.matrix(x_test)) %>% as.vector()
